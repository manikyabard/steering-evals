...
def test_magnitude_hypothesis():
    ...
    logger.info("Testing Magnitude-Based Steering Hypothesis")
    logger.info("=" * 50)
    logger.info(f"Model: {model_name}")
    logger.info(f"Testing alpha values: {alpha_values}")
    logger.info(f"Samples per alpha: {num_samples_per_alpha}")
...
def analyze_results(results):
    ...
    print("-" * 80)
    print(
        f"{'Alpha':^8} | {'|Alpha|':^8} | {'Mean':^8} | {'Std':^8} | {'Min':^6} | {'Max':^6} | {'N':^3}"
    )
    print("-" * 80)

    for stats in analysis_data:
        print(
            f"{stats['alpha']:^+8.2f} | {stats['abs_alpha']:^8.2f} | "
    ...
    for abs_alpha in sorted(magnitude_pairs.keys()):
        ...
        if len(pairs) == 2:  # We have both positive and negative
            ...
            combined_std = np.sqrt(
                neg_stats["std_length"] ** 2 + pos_stats["std_length"] ** 2
            )
    ...
    print(
        f"Larger |α| leads to longer reasoning in {length_increases}/{total_comparisons} "
...
def create_visualizations(results):
    ...
    length_by_alpha = [
        [r["thinking_length"] for r in results if r["alpha"] == a] for a in alpha_values
    ]
    ...
    pos_means = [
        (
            np.mean(magnitude_groups[m]["positive"])
            if magnitude_groups[m]["positive"]
            else 0
        )
        for m in magnitudes
    ]
    neg_means = [
        (
            np.mean(magnitude_groups[m]["negative"])
            if magnitude_groups[m]["negative"]
            else 0
        )
        for m in magnitudes
    ]
    ...
    plt.savefig(
        "steering_results/magnitude_hypothesis_analysis.png",
        dpi=300,
        bbox_inches="tight",
    )
    print(
...
def main():
    ...
    # Setup logging
    logger = setup_logging("test_magnitude_hypothesis")

    logger.info("Starting Magnitude-Based Steering Hypothesis Test")
    logger.info("This will test whether reasoning length follows |α| rather than α")

    # Run the experiment
    results = test_magnitude_hypothesis()

    if results is None:
        logger.error("Test failed - missing directions file")
        return
    ...
    logger.info("Magnitude hypothesis test complete!")
